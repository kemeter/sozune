...
- HTTPRoute `requestHeaderModifier` / `responseHeaderModifier` filters — set and remove request/response headers via Gateway API filters, mapped onto Sōzu's native frontend header edits. `add` is applied as `set` (Sōzu has no frontend append-without-replace) and logged. `urlRewrite`, `requestMirror` and `extensionRef` remain dropped with `Accepted=False reason=UnsupportedValue`.
...
- Listener-driven port binding, `parentRef.sectionName`/`port`, the `urlRewrite`/`requestMirror`/`extensionRef` HTTPRoute filters, and GRPCRoute/TCPRoute are not yet implemented — see [Kubernetes provider docs](documentation/providers/kubernetes.md#gateway-api-httproute) for the full support matrix.
