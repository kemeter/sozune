...
- HTTPRoute listener selection — a route's `parentRef` can now bind to a specific `Gateway` listener via `sectionName` (matched against a listener `name`) or `port`; a ref naming a listener the Gateway doesn't have is refused. A listener's `hostname` narrows which of a route's hostnames it serves (a listener on `*.example.com` drops hostnames outside that suffix, with wildcard matching). An unset `sectionName`/`port` still binds to the whole Gateway. Note: traffic is still served on the ports from `config.yaml`, not on each listener's own `port` — multi-port binding is a separate, planned change. See [HTTPRoute support](documentation/providers/kubernetes.md#gateway-api-httproute).
...
- Resolver-managed `domains` — a DNS-01 resolver can list `domains` (e.g. `*.example.com`) it provisions on its own, with no placeholder entrypoint. Sōzu serves the cert by SNI for every matching subdomain, so apps keep their exact-host routes and a busy domain no longer needs a per-host ACME order. Composes with the per-entrypoint `acme.resolver` binding; the resolver binding wins if a hostname is claimed by both.
...
- Client-IP matching — scope a route to specific client IPs or CIDRs with `sozune.http.<svc>.matchClientIP=<ip-or-cidr>,…`. Unlike the IP allow-list middleware (a `403` access filter), this is a routing matcher: a client outside the list gets `404`, as if the route didn't exist. See [Client-IP matching docs](documentation/routing/client-ip-matching.md).
...
- IP allow-list — per-route access filter: requests from a client IP not in the list are rejected with `403 Forbidden` before reaching auth, rate-limit, or the backend. `sozune.http.<svc>.ipAllowList=<ip-or-cidr>,…` (IPs auto-promoted to `/32`/`/128`), with secure `X-Forwarded-For` handling. Empty disables it. See [IP allow-list docs](documentation/middleware/ip-allow-list.md).
...
- WASM plugins — run a [http-wasm](https://http-wasm.io/) guest as request/response middleware. Declare a plugin in `config.yaml` (`plugins.<name>.path` + opaque `config`) and attach it per route with `sozune.http.<svc>.plugins=<name>` (comma-separated, ordered). Plugins can short-circuit a request (e.g. a `403`) or rewrite headers, and run after native middlewares but before compression. Each invocation is bounded by a wall-clock timeout and a memory limit. See [WASM plugins docs](documentation/middleware/wasm-plugins.md).
- WASM plugin outbound HTTP — guests may opt into two non-standard host extensions, `http_fetch` (blocking, for in-path decisions like a CrowdSec LAPI lookup) and `http_send` (fire-and-forget, for analytics beacons), restricted to an operator-declared `allowed_hosts`. Targets pointing at internal IP literals (loopback, RFC 1918 / unique-local, link-local incl. the `169.254.169.254` metadata endpoint) are refused unless the operator explicitly lists an internal target — a tenant can never widen the allow-list (anti-SSRF). See [Outbound HTTP](documentation/middleware/wasm-plugins.md#outbound-http-allowed_hosts).
- Per-route WASM plugin config — override a plugin's global config per entrypoint with `sozune.http.<svc>.plugins.<name>.<key>=<value>` labels (dotted keys nest; JSON-typed values keep their type), merged over the global default before the guest reads it. So the same plugin can run with different settings per app. Plugin-config values are redacted (`***`) in `GET /entrypoints`; malformed or L4-placed plugin labels are surfaced with a `W026` diagnostic. See [Per-route config](documentation/middleware/wasm-plugins.md#per-route-config).
...
- Consul Catalog provider — discover services from a [Consul](https://www.consul.io/) catalog. Enable with `providers.consul` (`endpoint`, optional `token`, `datacenter`, `poll_interval`); Sōzune maps service tags to `sozune.*` labels, filters to healthy instances, and uses Consul blocking queries so changes are picked up promptly. See [Consul provider docs](documentation/providers/consul.md).
...
- WASM plugin routes no longer silently truncate large bodies. A request or response body over the 1 MiB guest-buffer limit is now streamed through to the backend / client untouched (the guest just doesn't see it), instead of being dropped to an empty body. Works for both `Content-Length` and chunked transfers, since the decision is made from bytes actually read.
...
- `W026` invalid or ignored plugin-config label: no sub-key, nested past the depth limit, or set on a TCP/UDP route (plugins are HTTP-only)

### API

- New `GET /config` endpoint — admin-only read-only snapshot of the running configuration (listener ports, ACME settings, providers, dashboard and API listeners), with secrets omitted (the API user list and provider tokens are never serialized).
- New `GET /providers` endpoint returning the configured providers with their enabled state and live entrypoint counts.
